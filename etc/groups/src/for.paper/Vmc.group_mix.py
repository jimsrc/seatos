...
for varname in VARNAMES:
    ...
    ax.plot(i-di, sh0/avr[varname], 'v--', mec='blue', mfc='None', mew=1.5)
    ax.plot(i-di, sh1/avr[varname], 'o--', mec='green', mfc='None', mew=1.5)
    ax.plot(i-di, sh2/avr[varname], '^--', mec='red', mfc='None', mew=1.5)
    #--- mcs
    vals            = np.array([mc0, mc1, mc2])
    avr[varname]    = np.mean(vals)
    print " --> " + varname + 'MC: ', mc0, mc1, mc2, avr[varname], '\n'
    ax.plot(i+di, mc0/avr[varname], 'v--', color='blue')
    ax.plot(i+di, mc1/avr[varname], 'o--', color='green')
    ax.plot(i+di, mc2/avr[varname], '^--', color='red')
