...
type=str,
default='<final_histos>' if 'AUGER_REPO' not in os.environ else '{AUGER_REPO}/out/out.build_temp.corr/shape.ok_and_3pmt.ok/15min/histos_temp.corrected.h5'.format(**os.environ),
...
parser.add_argument(
'-as', '--inp_Auger_scals',
default='<final_scalers>' if 'PAO' not in os.environ else '{PAO}/data_auger/estudios_AoP/data/unir_con_presion/data_final_2006-2013.h5'.format(**os.environ),
help='.h5 file with Auger scalers (until pressure correction).'
)
parser.add_argument(
'-avr', '--avr',
type=str,
default='<rich-events>.nc' if 'ASO' not in os.environ else '{ASO}/icmes_richardson/data/rich_events2_ace.nc'.format(**os.environ),
help='.nc file with the average values inside diferent ICME sub-structures.'
)
parser.add_argument(
'-rc', '--rich_csv',
type=str,
default='<rich-table>.csv' if 'ASO' not in os.environ else '{ASO}/icmes_richardson/RichardsonList_until.2016.csv'.format(**os.environ),
