...
fname_fig   = '%s/figs_splitted_all.vars.png' % dir_figs
...
f   = plt.figure(1, figsize=(9, 15))
nr  = 1         # scale for row size
gs  = GridSpec(nrows=10*nr, ncols=2*3)
...
for i in range(3):
    ...
    for lmc, lsh in zip(fnro_mc, fnro_sh):
        ...
        #if varname in ('AlphaRatio.ACE', 'CRs.Auger', 'CRs.McMurdo', 'rmsB.ACE'):
            #continue
